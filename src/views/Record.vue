<template>
  <div>
    ...
    <p class="center" v-else-if="!categories.length">
      Категорий не найдено. <router-link to="/categories">Добавить новую категорию</router-link>
    </p>
    ...
    <form v-else class="form" @submit.prevent="onSubmit">
      <div class="input-field" >
        ...
        <label>Выберите каьегорию</label>
      </div>
      ...
      async onSubmit() {
        ...
        if (this.canCreateRecord) {
          try {
            ...
            const newRecord = await this.$store.dispatch('createRecord', formData)
            ...
            this.$msg(`Запись успешно создана`)
            this.$v.$reset()
            this.amount = 1
            this.description = ''
          } catch (e) {
          ...
          this.$msg(`недостаточно средств на счете (${this.amount - this.info.bill})`)
        }
